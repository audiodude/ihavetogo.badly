...
begin
    ...
    is_first_admin := not exists (select 1 from public.users where is_admin = true);

    -- Get default daily review limit from settings (admins are immune to limits)
    if is_first_admin then
        default_limit := null; -- Admins have no limit
    else
        select (value::integer) into default_limit
        from public.app_settings
        where key = 'default_daily_review_limit';

        -- Fallback to 1 if setting doesn't exist
        if default_limit is null then
            default_limit := 1;
        end if;
    end if;

    insert into public.users (id, email, is_admin, daily_review_limit)
    values (
        new.id,
        new.email,
        is_first_admin,
